...
const InboxTable = () => {
	...
	const handleReviewDeletion = async () => {
		try {
			toast.success(`User query successfully deleted`)
			setIsConfirmModelShow(false)
		} catch (error) {
			console.log(`User Query Deletion error! ${error.message}`)
		}
	}
					...
					<table className='w-full table-auto'>
						<thead>
							<tr className='bg-gray-2 text-left dark:bg-meta-4'>
								...
								<th className='min-w-[150px] py-4 px-4 font-medium text-black dark:text-black'>Subject</th>
						...
						<tbody>
							{!loadingQuery &&
								queryData?.map((query, key) => (
									<tr key={key}>
										<td className='border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11 flex items-center gap-3'>
											<h5 className='font-medium text-black dark:text-black'>{query?.name}</h5>
										</td>
										<td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
											<p className='text-black dark:text-black'>{query?.email}</p>
										</td>
										<td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
											<p className='text-black dark:text-black'>{query?.phone}</p>
										</td>
										<td className='text-black border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
											{query?.subject}
										</td>
										<td className='text-black border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
											<Moment fromNow>{query?.createdAt}</Moment>
										</td>
										<td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
											<div className='flex items-center space-x-3.5'>
												<Link href={`/dashboard/user-query/${query?._id}`} className='hover:text-primary'>
													<EyeSVG />
												</Link>
												<button
