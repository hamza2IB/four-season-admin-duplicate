...
import { GET_SINGLE_QUOTATION } from '@/app/graphql/queries'

function page({ params }) {
	const { loading: loadingQuery, data: dataQuery } = useQuery(GET_SINGLE_QUOTATION, {
		variables: { getSingleQuotationId: params.quotationID },
	})
	const router = useRouter()

	return (
		<>
			<Breadcrumb pageName='Inbox Detail' />
						...
						<div className='sticky flex items-center justify-between border-b border-stroke px-6 py-4.5 dark:border-strokedark'>
							<div className='flex flex-col' bis_skin_checked='1'>
								<h5 className='font-medium text-black dark:text-black mb-5 flex gap-x-12'>
									...
									{dataQuery?.getSingleQuotation?.subject || 'N/A'}
								</h5>
								<div className='flex items-center'>
									...
									<div bis_skin_checked='1'>
										<h5 className='font-medium text-black dark:text-black'>{dataQuery?.getSingleQuotation?.name}</h5>
										<p className='text-sm'>{dataQuery?.getSingleQuotation?.email}</p>
										<p className='text-sm'>{dataQuery?.getSingleQuotation?.phone}</p>
										<p className='text-sm'>{dataQuery?.getSingleQuotation?.engagement_model}</p>
										<p className='text-sm'>{dataQuery?.getSingleQuotation?.budget}</p>
									</div>
								</div>
							</div>
						</div>
						<div className='px-6 py-4.5'>
							<p className='dark:text-black mb-5 text-medium'>{dataQuery?.getSingleQuotation?.project_description || 'N/A'}</p>
						</div>
					</div>
				</div>
			</div>
