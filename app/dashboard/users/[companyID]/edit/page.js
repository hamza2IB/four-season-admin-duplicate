...
const FormLayout = ({ params }) => {
	...
	const getCompanyDetail = async () => {
		try {
			const { data } = await axios.get(`/company/${params.companyID}`)
			setCompanyLogoPreview(data?.logo)
					...
					<div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
						<Formik
							...
							initialValues={{
								...
								address: {
									country: companyDetail?.address?.country || '',
									state: companyDetail?.address?.state || '',
									city: companyDetail?.address?.city || '',
									streetAddress: companyDetail?.address?.streetAddress || '',
									postalCode: companyDetail?.address?.postalCode || '',
								},
								website: companyDetail?.website || '',
								phone: companyDetail?.phone || '',
								category: companyDetail?.category?._id || '',
								sociallinks: [
									{
										platform: 'Facebook',
										url:
											companyDetail?.sociallinks?.length > 0 && companyDetail?.sociallinks[0]
												? companyDetail?.sociallinks[0]?.url
												: '',
									},
									{
										platform: 'LinkedIn',
										url:
											companyDetail?.sociallinks?.length > 0 && companyDetail?.sociallinks[1]
												? companyDetail?.sociallinks[1]?.url
												: '',
									},
									{
										platform: 'Instagram',
										url:
											companyDetail?.sociallinks?.length > 0 && companyDetail?.sociallinks[2]
												? companyDetail?.sociallinks[2]?.url
												: '',
									},
									{
										platform: 'YouTube',
										url:
											companyDetail?.sociallinks?.length > 0 && companyDetail?.sociallinks[3]
												? companyDetail?.sociallinks[3]?.url
												: '',
									},
								],
							}}
							...
							onSubmit={async (values, { setSubmitting, resetForm }) => {
								try {
									await axios.patch(`/company/${companyDetail?._id}/edit`, values)
									getCompanyDetail()
									setSubmitting(false)
								} catch (error) {
									setSubmitting(false)
